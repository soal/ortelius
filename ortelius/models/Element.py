...
element_links = db.Table(
    'hm_element_links',
    db.Column('parent_element_id', db.Integer,
              db.ForeignKey('hm_elements.id')),
    db.Column('child_element_id', db.Integer,
              db.ForeignKey('hm_elements.id')),
    db.Column('connection_type', ENUM('ParentChild', 'Horisontal',
                                      name='connection_types'), nullable=False),
...
class Element(db.Model):
    ...
    element_type_id = db.Column(db.Integer, db.ForeignKey(
        'hm_element_types.id'), nullable=True, index=True)
...
element_type_info_fields = db.Table('hm_element_type_info_fields',
                                    db.Column('id', db.Integer,
                                              primary_key=True),
                                    db.Column('element_type_id', db.Integer,
                                              db.ForeignKey('hm_element_types.id')),
                                    db.Column('field_order', db.Integer),
                                    db.Column('field_name', db.String(
                                        255), nullable=False),
...
class ElementInfo(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    element_id = db.Column(db.Integer, db.ForeignKey(
        'hm_elements.id'), nullable=False)
    field_id = db.Column(db.Integer, db.ForeignKey(
        'hm_element_type_info_fields.id'), nullable=False)
...
class ElementType(db.Model):
    ...
    elements = db.relationship('Element', backref=db.backref(
        'element_type', uselist=False), lazy='dynamic')
